...
test("新ルール: 「金属アレルギー対応パーツを使用」の矛盾表現を検出する", () => {
  // 対応パーツ＝樹脂フックそのものなので、こう書いたうえで
  // 「樹脂フックへの変更もできます」と続けると二重・矛盾になる
  assert.ok(
    labelsFor(
      "ピアスは金属アレルギー対応パーツを使用しており、樹脂フックやイヤリングへの変更も承ります。",
    ).length > 0,
  );
  assert.ok(labelsFor("金属アレルギー対応パーツを使用しています").length > 0);

  // 正しい因果で書いた文は通す
  assert.deepEqual(
    labelsFor(
      "樹脂フックへの変更ができ、金属アレルギーの方にも安心してご使用いただけます。もちろん金属フックやイヤリングへの変更も承ります。",
    ),
    [],
  );
  // ブランド全体としての「金属アレルギー対応」は使ってよい
  assert.deepEqual(labelsFor("金属アレルギー対応のアクセサリーです"), []);
});
