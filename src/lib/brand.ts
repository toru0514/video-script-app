...
export const METAL_TYPES: Record<MetalType, { label: string; rule: string }> = {
  ...
  resin_option: {
    label: "樹脂フック・イヤリングへ変更可",
    rule: `この商品はフックを樹脂に変更できます。金属アレルギーに対応できるのは「樹脂フックに変えられるから」です。この因果を守って書いてください。
「金属不使用」とは書かないでください（標準では金属のフックを使うため）。
また「金属アレルギー対応パーツを使用しています」とは書かないでください。対応パーツとは樹脂フックそのものなので、そう書いたうえで「樹脂フックへの変更もできます」と続けると、同じことを二重に言う矛盾した文になります。
正しい書き方の例：「樹脂フックへの変更ができ、金属アレルギーの方にも安心してご使用いただけます。もちろん金属フックやイヤリングへの変更も承ります。」
（該当：ピアス・イヤリング）`,
  },
  metal: {
    label: "金属パーツ使用",
...
export const NG_RULES: NgRule[] = [
  ...
  },
  {
    ...
    label: "金属不使用の断定",
    replacement:
      "金属アレルギー対応（商品によって金属使用の有無が異なります／ピアスは樹脂フックに変更できます）",
    reason:
      "ブランド全体で金属を使っていないと読める断定は禁止です（ネクタイピン・カフス・ネックレスは金属パーツを使用します）。「金属アレルギー対応」「商品によって金属使用の有無が異なります」「ピアスは樹脂フックに変更できます」に置き換えてください。",
    exemptWhenMetalFree: true,
  },
  {
    // 「対応パーツ」＝樹脂フックそのもの。こう書いたうえで「樹脂フックへの変更もできます」と
    // 続けると同じことを二重に言う矛盾になる。因果（樹脂に変えられるから対応できる）で書かせる。
    pattern: /(金属)?アレルギー対応パーツ/,
    label: "「アレルギー対応パーツを使用」という言い方",
    replacement:
      "樹脂フックへの変更ができ、金属アレルギーの方にも安心してご使用いただけます（金属フックやイヤリングへの変更も承ります）",
    reason:
      "「金属アレルギー対応パーツ」とは樹脂フックそのものです。そう書いたうえで「樹脂フックへの変更もできます」と続けると、同じことを二重に言う矛盾した文になります。「樹脂フックへの変更ができるので、金属アレルギーの方にも安心してお使いいただけます」という因果で書き直してください。",
  },
  {
